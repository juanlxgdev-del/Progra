import tkinter as tk

def ventana():
    global ven1
    ven1 = tk.Tk()
    ven1.title("Ventana principal")
    ven1.geometry("400x300")
    ven1.config(bg="brown")

    eti1 = tk.Label(ven1, text="Esta es la ventana principal")
    eti1.pack()
    
    boton_ir_2 = tk.Button(ven1, text="Ventana 2", command=ventana_2)
    boton_ir_2.pack(pady=20)
    
    boton_ir_3 = tk.Button(ven1, text="Ventana 3", command=ventana_3)
    boton_ir_3.pack(pady=20)
    
    boton_ir_4 = tk.Button(ven1, text="Ventana 4", command=ventana_4)
    boton_ir_4.pack(pady=20)

    ven1.mainloop()

def destruir_ventana(ventana_actual):
    ventana_actual.destroy()
    ventana()
    ventana_2.destroy()
    ventana()
    ventana_3.destroy()
    ventana()
    ventana_4.destroy()
    ventana()

def ventana_2():
    global ven2
    ven1.destroy()

    ven2 = tk.Tk()
    ven2.title("Ventana 2")
    ven2.geometry("400x300")
    ven2.config(bg="yellow")

    eti2 = tk.Label(ven2, text="Ventana 2")
    eti2.pack()

    boton2 = tk.Button(ven2, text="Ventana principal",command=lambda: destruir_ventana(ven2))
    boton2.pack(pady=20)

    boton_ir_3 = tk.Button(ven2, text="Ventana 3", command=ventana_3)
    boton_ir_3.pack(pady=20)

    ven2.mainloop()

def ventana_3():
    global ven3

    ven3 = tk.Tk()
    ven3.title("Ventana 3")
    ven3.geometry("400x300")
    ven3.config(bg="black")

    eti3 = tk.Label(ven3, text="Ventana 3", fg="white", bg="black")
    eti3.pack()

    boton3 = tk.Button(ven3, text="Ventana principal",command=lambda: destruir_ventana(ven3))
    boton3.pack(pady=20)
    
    boton_ir_4 = tk.Button(ven3, text="Ventana 4", command=ventana_4)
    boton_ir_4.pack(pady=20)

    ven3.mainloop()

def ventana_4():
    global ven4

    ven4 = tk.Tk()
    ven4.title("Ventana 4")
    ven4.geometry("400x300")
    ven4.config(bg="green")

    eti4 = tk.Label(ven4, text="Ventana 4", fg="white", bg="green")
    eti4.pack()

    boton4 = tk.Button(ven4, text="Ventana principal",command=lambda: destruir_ventana(ven4))
    boton4.pack(pady=20)

    boton_ir_3 = tk.Button(ven4, text="Ventana 3",command=ventana_3)
    boton_ir_3.pack(pady=20)

    ven4.mainloop()

ventana()